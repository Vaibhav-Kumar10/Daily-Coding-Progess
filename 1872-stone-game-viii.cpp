...
public:
    int stoneGameVIII(vector<int>& stones) {
        ...
        vector<int> dp(n, 0);
        dp[n - 1] = prefixSum_score[n - 1];

        for (int ind = n - 2; ind > 0; ind--) {
            int player_take = prefixSum_score[ind];
            int opponent_score = dp[ind + 1];
            int score_dif_after_take = player_take - opponent_score;

            int player_skip = dp[ind + 1];
            dp[ind] = max(player_skip, score_dif_after_take);
        }
        return dp[1];
    }
    /*
